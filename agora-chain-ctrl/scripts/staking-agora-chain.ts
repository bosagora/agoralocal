...
import { BOACoin } from "../../scripts/utils/Amount";
import { GasPriceManager } from "../../scripts/utils/GasPriceManager";
...
async function main() {
    ...
    const deposit_data = JSON.parse(fs.readFileSync("./agora-chain-ctrl/validator_keys/deposit_data.json", "utf-8"));

    for (let idx = 0; idx < deposit_data.length; idx++) {
        console.log(`[${idx}] public key: ${deposit_data[idx].pubkey}`);

        const staker = accounts[0].address;
